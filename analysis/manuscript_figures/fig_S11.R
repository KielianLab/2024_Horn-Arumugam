...
## Kielian Lab data
## Code for Figure S11 in Horn et al., 2023
## Created: 2023-01-23
## Updated: 2023-04-11
...
## Figure S11A ----
# Load in data
compass_results <- read.csv(here("Figure S11", "Figure S11A.csv")) %>%
  group_by(subsystem) %>%
...
## Figure S11B ----
# Load in data
compass_results <- read.csv(here("Figure S11", "Figure S11B.csv")) %>%
  group_by(subsystem) %>%
  mutate(alpha = case_when(adjusted_pval > 0.1 ~ "N", adjusted_pval <= 0.1 ~ "Y"))
